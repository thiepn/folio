...
const navGroups: Array<{ label: string; items: Array<{ view: NavView; label: string; icon: IconName }> }> = [
  { label: 'Daily', items: [
...
export function Sidebar({ active, inboxCount, favoriteProjects, favoriteLists, favoriteTags, pinnedSmartViews, onNavigate, onOpenProject, onOpenList, onOpenTag, onOpenSmartView, onAppearance, onData }: {
  ...
  const hasFavorites = Boolean(favoriteProjects.length || favoriteLists.length || favoriteTags.length || pinnedSmartViews.length)
  return (
    <aside className="sidebar">
      ...
      <nav className="sidebar-nav" aria-label="Primary navigation">
        {navGroups.map((group) => <div className="sidebar-nav-group" role="group" aria-label={group.label} key={group.label}>
          <div className="sidebar-nav-group__label">{group.label}</div>
          {group.items.map((item) => (
            <button type="button" key={item.view} className={active === item.view ? 'is-active' : ''} aria-current={active === item.view ? 'page' : undefined} onClick={() => onNavigate(item.view)}>
              <Icon name={item.icon} />
      ...
      {hasFavorites ? <section className="sidebar-section">
      ...
      <div className="sidebar__footer">
        <div className="sidebar__local-status"><span className="sidebar__status-dot" />Local-first · sync optional</div>
        <button type="button" onClick={onData}><Icon name="download" /><span>Data & storage</span></button>
        <button type="button" onClick={onAppearance}><Icon name="settings" /><span>Appearance</span></button>
      </div>
    </aside>
  )
}
