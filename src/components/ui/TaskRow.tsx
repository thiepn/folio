        ...
        <div className="task-row__body">
          ...
          {(task.project || task.meta || task.subtaskTotal || task.activeBlockerCount) ? (
            <div className="task-row__meta">
              {[task.seriesId ? '↻ Recurring' : undefined, task.activeBlockerCount ? `⛓ Blocked by ${task.activeBlockerCount}` : undefined, task.project, task.meta, task.subtaskTotal ? `${task.subtaskCompleted}/${task.subtaskTotal} subtasks` : undefined].filter(Boolean).join(' · ')}
            </div>
          ) : null}
        </div>
