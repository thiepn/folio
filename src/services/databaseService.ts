...
const CONTENT_TABLES = [
  ...
  'focusSessions', 'recurringSeries', 'importBatches', 'patchBatches', 'calendarImportBatches', 'reviewRecords', 'reminders', 'reminderOccurrences', 'folders', 'lists', 'sections', 'tags',
] as const
...
async function migrateLegacyDatabaseName() {
  ...
  const [
    ...
    legacy.reminders.toArray(), legacy.reminderOccurrences.toArray(), legacy.folders.toArray(), legacy.lists.toArray(), legacy.sections.toArray(), legacy.tags.toArray(),
  ])
  ...
  await db.transaction('rw', [
    ...
    db.focusSessions, db.recurringSeries, db.settings, db.importBatches, db.patchBatches, db.calendarImportBatches, db.reviewRecords, db.reminders, db.reminderOccurrences, db.folders, db.lists, db.sections, db.tags,
      ...
      await Promise.all([
        ...
        db.importBatches.clear(), db.patchBatches.clear(), db.calendarImportBatches.clear(), db.reviewRecords.clear(), db.reminders.clear(), db.reminderOccurrences.clear(), db.folders.clear(), db.lists.clear(), db.sections.clear(), db.tags.clear(),
      ])
  ...
  const expected = [tasks, projects, habits, habitEntries, timeBlocks, dailyPlans, dailyPlanItems, focusSessions, recurringSeries, importBatches, patchBatches, calendarImportBatches, reviewRecords, reminders, reminderOccurrences, folders, lists, sections, tags].map((rows) => rows.length)
...
export async function getDatabaseHealth(): Promise<DatabaseHealth> {
  const [tasks, projects, habits, timeBlocks, dailyPlans, dailyPlanItems, focusSessions, recurringSeries, importBatches, patchBatches, calendarImportBatches, reviewRecords, reminders, reminderOccurrences, folders, lists, sections, tags] = await Promise.all([
    ...
    db.reminders.count(), db.reminderOccurrences.count(), db.folders.count(), db.lists.count(), db.sections.count(), db.tags.count(),
  ])
  const persistentStorage = navigator.storage?.persisted ? await navigator.storage.persisted() : null
  return {
    schemaVersion: DATABASE_SCHEMA_VERSION,
    counts: { tasks, projects, habits, timeBlocks, dailyPlans, dailyPlanItems, focusSessions, recurringSeries, importBatches, patchBatches, calendarImportBatches, reviewRecords, reminders, reminderOccurrences, folders, lists, sections, tags },
    persistentStorage,
  }
}
...
export async function loadDemoWorkspace(): Promise<DatabaseHealth> {
  const entityCounts = await Promise.all([
    ...
    db.dailyPlans.count(), db.dailyPlanItems.count(), db.focusSessions.count(), db.recurringSeries.count(), db.reviewRecords.count(), db.reminders.count(), db.reminderOccurrences.count(), db.folders.count(), db.lists.count(), db.sections.count(), db.tags.count(),
  ])
