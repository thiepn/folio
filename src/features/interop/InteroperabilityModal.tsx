    ...
    <Modal open={open} title="Portability & calendar" onClose={onClose} className="protocol-modal interop-modal">
      ...
      {tab === 'backup' ? <div className="protocol-stack">
        ...
        <section className="interop-section">
          <header><div><b>Export full backup</b><span>Schema v18 · complete planner state</span></div><Button variant="primary" onClick={() => void exportFullBackup()} disabled={busy}>Export JSON</Button></header>
        </section>
        <section className="interop-section">
          <header><div><b>Restore backup</b><span>Supported direct-restore range: schema v8–v18</span></div><input type="file" accept="application/json,.json" onChange={(event) => void loadBackupFile(event.target.files?.[0])} /></header>
          {restorePreview ? <>
            <div className="protocol-counts">
              <div><b>{restorePreview.counts.tasks}</b><span>Tasks</span></div><div><b>{restorePreview.counts.projects}</b><span>Projects</span></div><div><b>{restorePreview.counts.habits}</b><span>Habits</span></div><div><b>{restorePreview.counts.timeBlocks}</b><span>Time blocks</span></div><div><b>{restorePreview.counts.focusSessions}</b><span>Focus sessions</span></div><div><b>{restorePreview.counts.reviewRecords}</b><span>Reviews</span></div><div><b>{restorePreview.counts.reminders}</b><span>Reminders</span></div><div><b>{restorePreview.counts.reminderOccurrences}</b><span>Reminder events</span></div><div><b>v{restorePreview.backup.version}</b><span>Backup schema</span></div>
            </div>
