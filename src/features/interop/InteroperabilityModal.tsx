    ...
    <Modal open={open} title="Portability & calendar" onClose={onClose} className="protocol-modal interop-modal">
      ...
      {tab === 'backup' ? <div className="protocol-stack">
        ...
        <section className="interop-section">
          <header><div><b>Export full backup</b><span>Schema v17 · complete planner state</span></div><Button variant="primary" onClick={() => void exportFullBackup()} disabled={busy}>Export JSON</Button></header>
        </section>
        <section className="interop-section">
          <header><div><b>Restore backup</b><span>Supported direct-restore range: schema v8–v17</span></div><input type="file" accept="application/json,.json" onChange={(event) => void loadBackupFile(event.target.files?.[0])} /></header>
          {restorePreview ? <>
