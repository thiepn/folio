    ...
    <Modal open={open} title="Portability & calendar" onClose={onClose} className="protocol-modal interop-modal">
      ...
      {tab === 'backup' ? <div className="protocol-stack">
        ...
        <section className="interop-section">
          <header><div><b>Export full backup</b><span>Schema v24 · complete planner state + rich content · sync credentials excluded</span></div><Button variant="primary" onClick={() => void exportFullBackup()} disabled={busy}>Export JSON</Button></header>
        </section>
        <section className="interop-section">
          <header><div><b>Restore backup</b><span>Supported direct-restore range: schema v8–v24</span></div><input type="file" accept="application/json,.json" onChange={(event) => void loadBackupFile(event.target.files?.[0])} /></header>
          {restorePreview ? <>
            <div className="protocol-counts">
              <div><b>{restorePreview.counts.tasks}</b><span>Tasks</span></div><div><b>{restorePreview.counts.projects}</b><span>Projects</span></div><div><b>{restorePreview.counts.habits}</b><span>Habits</span></div><div><b>{restorePreview.counts.timeBlocks}</b><span>Time blocks</span></div><div><b>{restorePreview.counts.focusSessions}</b><span>Focus sessions</span></div><div><b>{restorePreview.counts.reviewRecords}</b><span>Reviews</span></div><div><b>{restorePreview.counts.reminders}</b><span>Reminders</span></div><div><b>{restorePreview.counts.reminderOccurrences}</b><span>Reminder events</span></div><div><b>{restorePreview.counts.lists}</b><span>Lists</span></div><div><b>{restorePreview.counts.sections}</b><span>Sections</span></div><div><b>{restorePreview.counts.tags}</b><span>Tags</span></div><div><b>{restorePreview.counts.notes}</b><span>Notes</span></div><div><b>{restorePreview.counts.attachments}</b><span>Attachments</span></div><div><b>{restorePreview.counts.habitGroups}</b><span>Habit groups</span></div><div><b>{restorePreview.counts.habitTemplates}</b><span>Habit templates</span></div><div><b>v{restorePreview.backup.version}</b><span>Backup schema</span></div>
            </div>
