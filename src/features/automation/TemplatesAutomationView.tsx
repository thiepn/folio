  ...
  return <div className="automation-workspace">
    ...
    {tab==='automations'?<div className="automation-two-col automation-rules-layout">
      <section className="automation-section"><div className="automation-section-head"><div><span className="eyebrow">Rules</span><h2>Automation rules</h2></div><span>{rules.length}</span></div>
        ...
        {testMessage?<div className="automation-test-result">{testMessage}</div>:null}<div className="automation-rule-list">{rules.map((rule)=><article key={rule.id}><header><div><strong>{rule.name}</strong><span>{triggerLabel(rule.trigger)} · {rule.actions.length} action{rule.actions.length===1?'':'s'}</span></div><label><input type="checkbox" checked={rule.enabled} onChange={(e)=>void automationService.setEnabled(rule.id,e.target.checked)}/><span>On</span></label></header><p>{rule.actions.map(actionLabel).join(' · ')}</p>{rule.lastRunAt?<small>Last run {new Intl.DateTimeFormat(undefined,{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(rule.lastRunAt))}</small>:null}<footer><Button onClick={()=>editRule(rule.id)}>Edit</Button><Button disabled={!manualTaskId} onClick={()=>void automationService.testRule(rule.id,manualTaskId,today).then((result)=>setTestMessage(result.matches?'Rule matches this task. '+result.actions.length+' action'+(result.actions.length===1?'':'s')+' would run.':'Rule does not match this task.')).catch((reason)=>setError(reason.message))}>Test</Button><Button disabled={!manualTaskId} onClick={()=>void automationService.runManual(rule.id,manualTaskId,today).then(onUndo).catch((reason)=>setError(reason.message))}>Run now</Button><Button onClick={()=>void automationService.removeRule(rule.id).then(onUndo)}>Delete</Button></footer></article>)}{!rules.length?<p>No automations yet.</p>:null}</div>
      </section>
