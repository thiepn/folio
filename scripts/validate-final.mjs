...
check('stable package version', pkg.version === '1.3.0', pkg.version)
...
check('database schema v13', /DATABASE_SCHEMA_VERSION\s*=\s*13\b/.test(database))
check('v12→v13 migration registered', database.includes('migrateV12ToV13'))
...
check('visible backup copy says v13', interop.includes('Schema v13 · complete planner state'))
check('visible restore range says v8–v13', interop.includes('schema v8–v13'))
check('runtime UI has no stale backup schema copy', !/Schema v1[12] · complete planner state|schema v8–v1[12]/.test(interop))

check('public v13 backup schema exists', exists('public/schema/folio-backup-v13.schema.json'))
for (const p of [
  'public/schema/folio-import-v1.schema.json',
  'public/schema/folio-patch-v1.schema.json',
  'public/schema/folio-backup-v13.schema.json',
