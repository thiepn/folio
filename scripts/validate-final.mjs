...
check('stable package version', pkg.version === '1.2.0', pkg.version)
...
check('database schema v12', /DATABASE_SCHEMA_VERSION\s*=\s*12\b/.test(database))
check('v11→v12 migration registered', database.includes('migrateV11ToV12'))
...
check('visible backup copy says v12', interop.includes('Schema v12 · complete planner state'))
check('visible restore range says v8–v12', interop.includes('schema v8–v12'))
check('runtime UI has no stale schema v11 copy', !/Schema v11|schema v8–v11/.test(interop))

check('public v12 backup schema exists', exists('public/schema/folio-backup-v12.schema.json'))
for (const p of [
  'public/schema/folio-import-v1.schema.json',
  'public/schema/folio-patch-v1.schema.json',
  'public/schema/folio-backup-v12.schema.json',
